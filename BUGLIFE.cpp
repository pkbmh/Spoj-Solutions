...
bool dfs(int u, int c, VI &vis, VI &col) {
    vis[u] = 1;
    col[u] = c;

    bool f = true;
    for(int v: g[u]) {
        if(vis[v] == -1) f = f & dfs(v, 1-c, vis, col);
...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    for(int t=1; t <= tt; ++t) {
        ...
        if(!f) cout <<"Scenario #" << t << ":" << nl << "Suspicious bugs found!"  << endl;
        else cout << "Scenario #" << t << ":" <<  nl << "No suspicious bugs found!" << endl;
