...
LLI solve(VI &arr, int a, int i, int j) {
    if (i == j) return a * arr[i];
    if(dp[i][j] != -1) return dp[i][j];
    return dp[i][j] = max(solve(arr, a+1, i+1, j) + arr[i]*a, solve(arr, a+1, i, j-1)+arr[j]*a);
}
int main() {
