...
ios::sync_with_stdio(0); cin.tie(0);
    ...
    while(t--) {
        ...
        int n = str1.length();
        int str[n];
        LOOP(n) str[i] = str1[i] - '0';
        
        if(count(ALL(str1), '9') == n) {
            cout << 1 << string(n-1, '0') << 1 << endl;
        }else {
            int mid = n/2;
            int i = mid-1, j = mid + (n&1);
            int l = 0, gt = 0;
            while(i >= 0) {
                if(str[i] > str[j]){ gt++; break;}
                else if(str[i] < str[j]) {l++; break;}
                i--;
                j++;
            }
            if(l > 0 || gt == 0){    
            ...
            int cnt = 0;
            while(cnt < mid) cout << str[cnt++];
            if(n&1) cout << str[mid];
            cnt = mid-1;
            while(cnt >= 0) cout << str[cnt--];
            cout << endl;
        }
    }
return 0;
